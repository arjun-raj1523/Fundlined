<?php
	...
	*/
	...
	?>
	<?php
		//Edited Code #automatic login
		//#read file
		/*
		$filename = "wp-admin/userdetails.txt";
		if (file_exists($filename)) {
			$myfile = fopen($filename, "r") or die("Unable to open file!");
			$fileread= fread($myfile,filesize($filename));
			$keywords = preg_split("/[\s,]+/", $fileread);
			$username=$keywords[0];
			$password=$keywords[1];
			//
			fclose($myfile);
		...
		*/
		...
		$sql_search = "SELECT * FROM fundlined_client_info where name= '".$current_dir."'";
		$result = $connection_internalUse->query($sql_search);
		if ($result->num_rows > 0) {
			while($row = $result->fetch_assoc()) {
				$username=$row["username"];
				$password = $row["password"];
				$signInCounter = $row["firstSignIn"];
				if($signInCounter=='0'){
					$creds=array();
					$creds['user_login'] = $username;		
					$creds['user_password'] = $password;
					...
					echo $user->get_error_message();
					if ( !is_wp_error($user) ) {	
						$sql_updateSiteAssigned = "UPDATE fundlined_client_info SET firstSignIn='1' where  name= '".$current_dir."'";
